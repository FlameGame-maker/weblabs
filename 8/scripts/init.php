...
function unauthorized() {
  return array(
    'headers' => array('HTTP/1.1 401 Unathorized'),
    'entity' => theme('401'),
  );
}
...
function theme($t, $c = array()) {
  ...
  if (!file_exists($template)) {
    $to_string = function($item) use (&$to_string) {
      ...
      if (is_array($item)) return implode('', array_map($to_string, $item));
      if (is_object($item) && method_exists($item, '__toString')) return $item->__toString();
      return '';
    };
    return implode('', array_map($to_string, $c));
  }

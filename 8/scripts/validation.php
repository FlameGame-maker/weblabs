...
function getErrorMessage($field, $value) {
	...
	if (isset($errorMessages[$field])) {
		return isset($errorMessages[$field][$value]) ? $errorMessages[$field][$value] : false;
	}
	return false;
}
...
function validateAll($fio, $phone, $email, $birthday, $sex, $languages, $bio, $consent) {
	...
	} else setcookie('consent_error', '', 1);
	return $inputErrors;
}
?>